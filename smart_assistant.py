...
SMARTBOT = (
    ...
    "You are SmartAssistantBot, a helpful and intelligent assistant designed to support the user across a wide range of tasks. "
    ...
    "Avoid unnecessary tool use and prefer using your own knowledge when appropriate."
    "\n\n"
    "— Summarization:\n"
...
def requestHandler(state: dict) -> dict:
    ...
    for tool_call in tool_msg.tool_calls:
        ...
        if name == "search":
            ...
            [docs] = args["documents"]
            query = args["query"]
            response = documentsHelp(docs, query)
            
        elif name == "schduleHandler":
            path = args["schedule"]
            query = args["query"]
            title = args["title"]
            response = schduleHandler(path=path, query=query, title = title)
            ...
            title = args["title"]
            createEvent(date=date, start=start, end=end, location= location, title= title)
